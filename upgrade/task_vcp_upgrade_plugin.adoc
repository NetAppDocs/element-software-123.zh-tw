...
NOTE: 管理服務2.22.7包含NetApp Element VMware vCenter Server 5.0的VMware vCenter外掛程式、其中包含遠端外掛程式。如果您使用Element外掛程式、則應升級至5.0版、以符合VMware指令、該指令會移除對本機外掛程式的支援。 https://kb.vmware.com/s/article/87880["深入瞭解"^]。如果您不使用外掛程式、這是選擇性的升級。

[role="tabbed-block"]
====
.Element vCenter外掛程式5.0或更新版本
--
從Element Plug-in for VMware vCenter Server 5.0開始、此升級程序涵蓋下列升級案例：
...
. 選取* vCenter外掛程式登錄*。
+
** vCenter Server 5.0元素外掛程式的vCenter外掛程式登錄頁面：
+
