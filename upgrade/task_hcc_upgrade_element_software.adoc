...
IMPORTANT: 如果您要將H610S系列節點升級至Element 12.3.x、且節點執行的元件版本早於11.8、則需要執行其他升級步驟（<<Upgrading H610S storage nodes to Element 12.3.x (phase 2),第2階段>>）。如果您執行的是元素11.8或更新版本、則不需要執行其他升級步驟（階段2）。
...
.您需要的是 #8217 ；需要的是什麼
* 如果您的管理節點未連線至網際網路、您已從下列位置下載套件：
+
https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab["Element軟體升級套件（.tar.gz）、適用於SolidFire 各種儲存叢集"^]
...
| 無法偵測 | 當NetApp混合雲控制系統無法連線至線上軟體儲存庫時、會顯示此狀態、而非*可用版本*。如果您有外部連線功能、但仍看到此訊息、請檢查您的 link:../mnode/task_mnode_configure_proxy_server.html["Proxy組態"]。 
...
... 請參閱 <<Upgrading H610S storage nodes to Element 12.3.x (phase 2)>> 並完成每個節點的程序。
... 視需要重新驗證、然後選取*「PGE」（更新）/「｛upgradeId｝」*。
... 選擇*試用*。
... 在* upgradeId*中輸入上一步的升級ID。
...
. 請下載 https://library.netapp.com/ecm/ecm_get_file/ECMLP2840740["Json 檔案"^] (https://library.netapp.com/ecm/ecm_get_file/ECMLP2840740)[] 從非管理節點的電腦上的NetApp支援網站、將其重新命名為「metadata.json」。
